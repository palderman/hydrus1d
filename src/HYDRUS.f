...
*||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*
      ...
      subroutine HYDRUS() bind(C, name = "hydrus")
...
*-----------------------------------------------------------------------
      ...
      cFileName = cDataPath(1:iLengthPath)//slash//'Options.in'
...
*     Initialization
     ...
     !          OmegaW,lEnd,lVaporOut,lFluxOut)
      ...
      cFileName = cDataPath(1:iLengthPath)//slash//'Selector.in'
      open(30,file=cFileName, status='old',err=901)
      cFileName = cDataPath(1:iLengthPath)//slash//'Profile.dat'
      open(32,file=cFileName, status='old',err=901)
      cFileName = cDataPath(1:iLengthPath)//slash//'I_CHECK.OUT'
      open(50,file=cFileName, status='unknown',err=902)
      if(lPrint) then
        cFileName = cDataPath(1:iLengthPath)//slash//'RUN_INF.OUT'
        open(70,file=cFileName, status='unknown',err=902)
        cFileName = cDataPath(1:iLengthPath)//slash//'T_LEVEL.OUT'
        open(71,file=cFileName, status='unknown',err=902)
        cFileName = cDataPath(1:iLengthPath)//slash//'NOD_INF.OUT'
        open(75,file=cFileName, status='unknown',err=902)
        cFileName = cDataPath(1:iLengthPath)//slash//'BALANCE.OUT'
        open(76,file=cFileName, status='unknown',err=902)
        cFileName = cDataPath(1:iLengthPath)//slash//'OBS_NODE.OUT'
        open(77,file=cFileName, status='unknown',err=902)
        cFileName = cDataPath(1:iLengthPath)//slash//'PROFILE.OUT'
...
*     Read input data --------------------------------------------------
     ...
     !            iVer,lPrint,lCentrif,lSnow,hSeep,lFlux,lActRSU,err)
      ...
      if(lPrint) then
        if(lFluxOut) then
          cFileName = cDataPath(1:iLengthPath)//slash//'T_Level1.OUT'
          open(44,file=cFileName, status='unknown',err=902)
        end if
        if(lVaporOut.and.lVapor) then
          cFileName = cDataPath(1:iLengthPath)//slash//'NOD_INF_V.OUT'
          open(45,file=cFileName, status='unknown',err=902)
        end if
      end if
      if(TopInF.or.BotInF.or.AtmBC) then
        cFileName = cDataPath(1:iLengthPath)//slash//'ATMOSPH.IN'
        open(31,file=cFileName, status='old',err=901)
        if(lPrint) then
          cFileName = cDataPath(1:iLengthPath)//slash//'A_LEVEL.OUT'
          open(72,file=cFileName, status='unknown',err=902)
        end if
        if(lMeteo) then
          cFileName = cDataPath(1:iLengthPath)//slash//'Meteo.in'
          open(33,file=cFileName, status='old',err=902)
          if(lPrint) then
            cFileName = cDataPath(1:iLengthPath)//slash//'Meteo.out'
     ...
     !              cDataPath)
      if(iModel.eq.nTabMod) then
        cFileName = cDataPath(1:iLengthPath)//slash//'Mater.in'
...
************************************************************************
     ...
     !temperature measurements!'
      ...
      cFileNameErr = cDataPath(1:iLengthPath)//slash//'Error.msg'
